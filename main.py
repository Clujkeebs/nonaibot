...
def main() -> None:
    ...
    # Immediate warm-up: regime + first scans
    try:
        log.info("Warm-up: updating regime...")
        engine.update_regime()
        log.info("Warm-up: checking exits on existing positions...")
        engine.check_all_exits()
        log.info("Warm-up: initial crypto scan...")
        engine.run_crypto_strategies()
        log.info("Warm-up: initial equity scan...")
        engine.run_equity_strategies()
